...
```
  ...
  * The problem with this is that the data in the call is a "language" object; we can't simply substitute `xtest$call$G` because it's not an integer in the way the expression `1:5` is. There is probably a way to make it work, but I am not certain how. Another way to get the values of *G* in the original call is to use the row names in the data.frame made from `xtest$BIC`. 
  
```{r}
as.integer(row.names(bictbl))
```
